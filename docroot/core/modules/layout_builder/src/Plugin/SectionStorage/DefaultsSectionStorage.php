...
class DefaultsSectionStorage extends SectionStorageBase implements ContainerFactoryPluginInterface, DefaultsSectionStorageInterface {
  ...
  public function deriveContextsFromRoute($value, $definition, $name, array $defaults) {
    ...
    if (isset($defaults['langcode'])) {
      $language = $this->languageManager->getLanguage($defaults['langcode']);
  ...
  public function save() {
    if ($this->getTranslationLanguage()) {
      $config_override = $this->translationOverride->save();
      return $config_override->isNew() ? SAVED_NEW : SAVED_UPDATED;
    }
  ...
  public function getTranslatedComponentConfiguration($uuid) {
    if ($sections_override = $this->translationOverride->get('third_party_settings.layout_builder.sections')) {
      foreach ($sections_override as $section) {
