...
class GenerateTheme
{
  ...
  public function setParameters() {
    ...
    $defaultValues = array(
      ...
      'machine-name' => 'rocketship_theme_starter',
      'preset' => 'starter',
      'theme-path' => 'dist',
      'author' => 'rembrandx',
    ...
    $contents = array(
      ...
      'kit_files_temp' => array(
        ...
        '/' . 'STARTER.info.yml.gtemp',
        '/' . 'STARTER.libraries.yml.gtemp',
        '/' . 'STARTER.layouts.yml.gtemp',
        '/' . 'STARTER.breakpoints.yml.gtemp',
        '/' . 'STARTER.theme.gtemp',
      )
    );
  ...
  private function copyKitFilesDirs() {
    ...
    // recursively copy the STARTER folders first
    foreach ($this->kit_dirs as $kit_dir) {

      $source = $this->skeleton_dirs . '/STARTER/' . $kit_dir;
    ...
    // copy STARTER files if they don't exist in destination yet
    foreach ($this->kit_files as $kit_file) {

      $source = $this->skeleton_dirs . '/STARTER/' . $kit_file;
    ...
    // ** copy more root files from STARTER, which are template-based (.gtemp)
    ...
    $this->copyr([
      'source' => $this->skeleton_dirs . '/STARTER' . '/storybook-templates/' . 'webpack.config.js.gtemp',
  ...
  private function copyr(array $parameters)
  {
    ...
    if (is_file($source)) {
      ...
      if (isset($path_parts) && isset($path_parts['extension']) && $path_parts['extension'] === 'gtemp') {
        ...
        if (strpos($file_name, 'STARTER')) {
          $file_name = str_replace('STARTER', $this->machine_name, $file_name);
        }
        ...
        if ($this->startsWith($file_name, 'STARTER')) {
          $file_name = str_replace('STARTER', $this->machine_name, $file_name);
        }
  ...
  public function init()
  {
    ...
    // ** copy the rest of the theme files from STARTER + whatever preset folder is chosen
