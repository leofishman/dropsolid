...
(function ($, Drupal, window, document) {
  ...
  self.focusFields = function (input, textarea, select) {
    ...
    if (input.length) {
      // iterate all inputs
      once('js-once-input', input).forEach(function(inputElement) {
    ...
    if (textarea.length) {
      once('js-once-textarea', textarea).forEach(function(textareaElement) {
    ...
    if (select.length) {
      once('js-once-select', select).forEach(function(selectElement) {
  ...
  self.customSelect = function (select) {
    once('js-once-select-wrap', select).forEach(function (selectElement) {
      if ($(selectElement).closest('.form__dropdown').length < 1) {
        $(selectElement).wrap('<div class="form__dropdown"></div>');
      }
    });
  };
  ...
  self.textareaScroll = function (textarea) {
    ...
    once('js-once-textarea-scroll', textarea).forEach(function(textareaElement) {
